...
public class Servicecommentaire {
    ...
    public void add(Commentaire c) {
        try {
            String req = "INSERT INTO `commentaire`(`id_user`, `id_pub`, `contenu`) VALUES (?,?,?)";
    ...
    public Commentaire getById(int id) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                commentaire.setId_user(rs.getInt("id_user"));
    ...
    public void edit(Commentaire c) {
        try {
            String req = "UPDATE `commentaire` SET `id_user`=?, `id_pub`=?, `contenu`=? WHERE `id`=?";
            PreparedStatement ps = cnx.prepareStatement(req);
            ps.setInt(1, c.getId_user());
            ps.setInt(2, c.getId_pub());
            ps.setString(3, c.getContenu());
